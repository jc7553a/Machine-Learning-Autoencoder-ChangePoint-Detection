...
'''
    JP Clark
    Implement OCSVM for Shuttle Data Set
'''
...
nu = len(testingData)/(len(testingData)+len(shuttleData))

print("Starting OCSVM")
clf = svm.OneClassSVM(nu = nu, kernel = 'rbf', gamma = .00005)
clf.fit(shuttleData)

losses = clf.decision_function(testingData)
